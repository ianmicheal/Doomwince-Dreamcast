...
//
// V_MarkRect //11.4.98 dlw this isn't used anywhere...
// 
...
// this probably isn't needed anymore  note to disable later
#ifdef RANGECHECK 
...
// V_DrawPatchDouble
// Masks a column based masked pic to the screen. 
// 10.10.98 - dlw: for large menus at 640x400
void V_DrawPatchDouble ( int x, int y, int scrn, patch_t* patch )
{ 
    int		count;
    int		col; 
    column_t*	column; 
...
//        fprintf( stderr, "V_DrawPatch: bad patch (ignored)\n");
    ...
    // for ( ; col<w ; x++, col++, desttop++)  x not used
	...
	{ 
		...
		{ 
            source = (byte *)column + 3; 
            dest = desttop + column->topdelta*(SCREENWIDTH*2);
			count = column->length;
            //sprintf(MsgText, "count = %d\n", count);
            //WriteDebug(MsgText);
            ...
            while (count--) 
			{ 
                *dest = *source++; // first pixel
				*(dest+1) = *dest; // immediate right
				*(dest+SCREENWIDTH) = *dest; // immediate below
				*(dest+SCREENWIDTH+1) = *dest; // immediate below&right
                dest += (SCREENWIDTH*2); 
			} 
            column = (column_t *)((byte *)column + column->length + 4 );
		} 
	}
    //WriteDebug("Finished V_DrawPatchDouble...\n");
} 
...
*/ 
//11.4.98 dlw opt-> drawblock only called once with fixed params so...
//#ifdef RANGECHECK 
...
/* 11.5.98 only called once in whole code so optimized dlw    
...
*/
void V_DrawBlock(void)
{ 
	memcpy(screens[0], screens[2], SCREENMULT);
} 
