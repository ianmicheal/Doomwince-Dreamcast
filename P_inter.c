  ...
  mobj_t*	toucher )
{
	...
	{
		player->itemcount++;
		++totalscore;  // score keeping 1 point per item
	}
  ...
  int 		damage )
{
    ...
    fixed_t	thrust;
    //UNUSED: int		temp;
	static char localscoretext[52]; //ST_MSGWIDTH
	    ...
	    || source->player->readyweapon != wp_chainsaw))
    {
		...
		target->momx += FixedMul (thrust, _finecosine(ang));
		target->momy += FixedMul (thrust, _finesine(ang));
    }
    ...
    if(player)
    {
		...
		{
			//screw up cheaters score <-- original score testing
			if(player->cheats & CF_GODMODE) totalscore = 0;
			return;
		}
		if (player->armortype)
		{
		...
		//UNUSED: temp = damage < 100 ? damage : 100;
		//UNUSED: if (player == &players[consoleplayer])I_Tactile(40,10,40+temp*2);
