  ...
  int		x2 )
{
    ...
    ds_xfrac = viewx + FixedMul(_finecosine(angle), length);
    ds_yfrac = -viewy - FixedMul(_finesine(angle), length);

/*
    angle = (viewangle + xtoviewangle[x1])>>(ANGLETOFINESHIFT-3);
    {
        float   sine, cosine;
        __fsca(&sine, &cosine, angle);
        ds_xfrac = viewx + (fixed_t)(cosine * length);
        ds_yfrac = -viewy - (fixed_t)(sine * length);
    }
*/
    if (fixedcolormap)
...
void R_ClearPlanes(void)
   {
    ...
    basexscale = FixedDiv (_finecosine(angle),centerxfrac);
    baseyscale = -FixedDiv (_finesine(angle),centerxfrac);
   }
...
void R_DrawPlanes(void)
{
    ...
    for(pl = visplanes; pl < lastvisplane; pl++)
	{
		...
		{
            //11.8.98 dlw-since detail/detailshift are always
			// zero in this thing...
