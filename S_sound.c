  ...
  int           musicVolume )
{
  ...
  //WriteDebug("S_Init: default sfx volume %d\n", sfxVolume);
...
//          / S_ATTENUATOR;
		*vol = 15+(((snd_SfxVolume-15)*FixedDiv(S_CLIPPING_DIST - approx_dist, S_ATTENUATOR))>>FRACBITS);
	}
	else
	{
		// distance effect
//      *vol = (snd_SfxVolume
//              * (S_CLIPPING_DIST - approx_dist)) / S_ATTENUATOR;
		*vol = ((snd_SfxVolume*FixedDiv(S_CLIPPING_DIST - approx_dist, S_ATTENUATOR))>>FRACBITS);
	}
