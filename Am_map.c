...
#define FTOM(x) (fixed_t)((float)(x) * (float)(scale_ftom))
#define MTOF(x) (fixed_t)((float)(x) * (float)(scale_mtof) * 0.00000000023283064365386962890625f)
  ...
  angle_t	a )
{
    ...
    tmpx =
	FixedMul(*x,_finecosine(a>>ANGLETOFINESHIFT))
	- FixedMul(*y,_finesine(a>>ANGLETOFINESHIFT));
    
    *y   =
	FixedMul(*x,_finesine(a>>ANGLETOFINESHIFT))
	+ FixedMul(*y,_finecosine(a>>ANGLETOFINESHIFT));
