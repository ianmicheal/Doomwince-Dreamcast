...
void I_InitNetwork(void)
{
    ...
    netget = PacketGet;
    netgame = true;
	internetgame = false; //11.12.98 dlw new internet stuff

    
	//11.12.98 dlw--this is a crap out, but a decent way to get some
	//speed during internet play.
	
	// parse player number and host list
    doomcom->consoleplayer = myargv[i+1][0]-'1';
	//11.12.98 >4 players indicates wish for internet speed
	if(doomcom->consoleplayer > 3)
	{
		internetgame = true;
		doomcom->consoleplayer -=4; //readjust to 0-3
		WriteDebug("Internet play requested: player number adjusted.\n");
	}
