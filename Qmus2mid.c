...
size_t fwrite2(int2 *ptr, size_t size, FILE *file)
{
  int4 rev = 0;
  int i;
...
void TWriteByte( char MIDItrack, char byte, struct Track track[] )
{
  ...
  if( pos < TRACKBUFFERSIZE )
    ...
    {
      WriteDebug("ERROR : Track buffer full.\n"
...
void TWriteVarLen( int tracknum, register int4 value, struct Track track[] )
{
...
int WriteMIDheader( int2 ntrks, int2 division, FILE *file )
{
  ...
  ntrks <<= 8;
  fwrite(&ntrks, 2, 1, file);
  // fwrite2( &division, 2, file );
  division <<= 8;
  fwrite(&division, 2, 1, file);
  return 0 ;
}
...
void WriteTrack( int tracknum, FILE *file, struct Track track[] )
{
  ...
  size2 = (int2) track[tracknum].current+4;
  fwrite( "MTrk", 4, 1, file ) ;
  if( !tracknum ) size2 += 31; //orig code had 33  //31 diff ??

  //fwrite2( &size, 4, file );  //<<<<trying to get normal val
  size=0;
  fwrite( &size, 2, 1, file);
  size=size2;
  size >>=8;
  fwrite( &size, sizeof(char), 1, file);
  fwrite( &size2, sizeof(char), 1, file);
  if( !tracknum)
	  fwrite( TRACKMAGIC1 "Quick MUS->MID ! by S.Bacqu", 31, 1, file ) ;
...
void WriteFirstTrack( FILE *file )
{
  ...
  size = 43;
  fwrite( "MTrk", 4, 1, file );
  size<<=24;  //this somehow puts the data in the right hole
  fwrite( &size, 4, 1, file);
  // fwrite2( &size, 4, file ) ;  // was 4 testing size,4,file
...
// 10.10.98 here's another one thats gonna get ripped apart for
// windows exclusive use
             ...
             int2 division, int BufferSize, int nocomp )
{
	...
	int4 DeltaTime, TotalTime = 0, n = 0;
	...
	if( (file_mus = fopen( "doomsong.mus", "rb" )) == NULL )
		return COMUSFILE;
	
	// stat( mus, &file_data ); <<<---stat for unix mainly

	if( (file_mid = fopen( "doomsong.mid", "wb" )) == NULL )
    {
		Close();
		return COTMPFILE;
    }
...
// 10.10.98 this is gonna be badly cracked to just quick convert
// all the extra checking stripped cuz--hell it's windows anyway
            ...
            int size, int nocomp, int *ow )
{
	...
	//if( access( "doomsong.mus", 0 ) )
    //{
    //  WriteDebug( "ERROR: Could not locate temporary music file.");
    //  return 1;
    //}
	
	// qmus2mid( const char *mus, const char *mid, int nodisplay,
    //         int2 division, int BufferSize, int nocomp )
	// error = qmus2mid( mus, mid, nodisplay, div, size, nocomp ) ;
	error = qmus2mid( "blah", "blah", 0, 0, 0, 0);

	if( error )
    {
		WriteDebug( "ERROR Converting mus file: " );
		switch( error )
        {
        case NOTMUSFILE :
			WriteDebug( "doomsong.mus is not a MUS file.\n"); break;
        case COMUSFILE :
			WriteDebug( "Can't open doomsong.mus for read.\n");	break;
        case COTMPFILE :
			WriteDebug( "Can't open temp file.\n");	break;
        case CWMIDFILE :
			WriteDebug( "Can't write midi file.\n"); break;
        case MUSFILECOR :
			WriteDebug( "Midi file is corrupted.\n"); break;
        case TOOMCHAN :
			WriteDebug( "Midi file contains more than 16 channels.\n"); break;
        case MEMALLOC :
			WriteDebug( "Not enough memory.\n" ); break;
        default : break;
        }
		return 1;
    }
